...
from app.models.notification_settings import NotificatioSettings
...
@settings_router.post("/add-notification-settings", response_model=ActionResult)
async def add_notification_settings(settings: NotificatioSettings):
    action_result = await db.add_entity(settings)
    return action_result
